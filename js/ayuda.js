// Ayuda nativa — enlaces a integración y resumen de flujo Hydro Cannabis
...
function renderAyudaPage() {
  ...
  host.innerHTML = `
    <div class="card">
      ...
      <p class="body-prose">Esta app prioriza <strong>cannabis en hidro</strong> con núcleo <strong>RDWC/DWC</strong> (Medir, variedades, calendario por cepa); NFT, mesa flotante y aeroponía siguen disponibles con aviso de mayor complejidad. Los módulos extendidos (torre, riego por pulsos avanzado, meteo alimentario) se abren desde <strong>Más → Integración</strong> o desde las tarjetas de integración en Cultivo / Calendario / Historial.</p>
      ...
      <div class="ayuda-action-grid">
        <button type="button" class="btn btn-primary" onclick="navToHcEmbed('ayuda')"><i class="ti ti-book"></i> Manual completo (integración)</button>
    ...
    <div class="card">
      ...
      <p class="body-prose body-prose--tight">En cada guardado del cultivo se actualiza un resumen interno (<code>fusion.growContext</code>) con sistema, emplazamiento, depósito, sitios, recinto, prioridad de port (<code>hydroCannabisPortTier</code>) y etiqueta de la instalación activa, para alinear datos entre pantallas nativas y la vista de integración. Si el pronóstico en <strong>Climatología</strong> es reciente y coincide la rejilla, <strong>Riego</strong> reutiliza esa serie horaria para ET₀/VPD sin repetir la misma petición a la API.</p>
    </div>
  `;
}
